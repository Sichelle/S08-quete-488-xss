  ...
  describe("when authenticated", () => {
    ...
    describe("POST /messages", () => {
      describe("with non-empty content", () => {
        describe("with JavaScript in personalWebsiteURL", () => {
          it("responds with error", async done => {
            await agent
              .post("/messages")
              .send(
                "content=Hola&personalWebsiteURL=javascript:alert('Hacked!');"
              )
        ...
        describe("with HTTP URL in personalWebsiteURL", () => {
          it("responds with success", async done => {
            await agent
              .post("/messages")
              .send(
                "content=Hola&personalWebsiteURL=https://giphy.com/stories/pandamonium-0cab7f0b-e7f5"
              )
              .expect("Content-Type", "application/json; charset=utf-8")
              .expect(201);
            done();
